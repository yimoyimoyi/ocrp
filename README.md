...
ctranslate2 的 CUDA 卷积层推理依赖 cuDNN 8 的 `cudnn_ops_infer64_8.dll`（Windows）/ `libcudnn_ops_infer.so.8`（Linux）。若缺失，ASR 自动回退 CPU 模式（不影响功能，仅速度较慢）。

**手动安装**：从 NVIDIA 开发者网站下载 cuDNN 8 for CUDA 12，将所需 DLL 放入 `models/asr/lib/`：

1. 访问 https://developer.nvidia.com/cudnn （需注册免费账号）
2. 下载 **cuDNN 8.9 for CUDA 12.x**（Windows: zip, Linux: tar.xz）
3. 解压后，将以下文件复制到 `models/asr/lib/`：
...
```
models/asr/lib/
├── cudnn_ops_infer64_8.dll    # Windows - 基础 ops
├── cudnn_cnn_infer64_8.dll    # Windows - 卷积推理
├── cudnn64_8.dll              # Windows - 运行时
├── libcudnn_ops_infer.so.8    # Linux
├── libcudnn_cnn_infer.so.8    # Linux
└── libcudnn.so.8              # Linux
```

> 💡 启动后日志会显示 `cuDNN 8 found (lib)` 即表示 GPU ASR 已就绪。
