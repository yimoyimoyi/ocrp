...
- **Config system**: `core/config_manager.py` handles all JSON config files with comment support (`//` and `/* */`). Settings auto-migrate between versions. `config/settings.json` is gitignored — it persists UI state (window geometry, last engine, mode params).
